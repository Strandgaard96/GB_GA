...
def compute_dft(
    ...
    os.mkdir(dir)
    os.chdir(dir)
    file_name = write_gaussian_input_file(
        mol, "tsguess", constr=constr, command=command, cpus=cpus
    )
...
def make_gaussian_constrained_opt(xyz_file, bond_pairs_changed, charge):
    ...
    with open(com_file, "w") as _file:
        ...
        _file.write(
            "#opt external='~/soft/gau_xtb/xtb.sh' int=ultrafine "
        ...
        p = subprocess.Popen(
            f"submit_gaussian_legacy {com_file}",
            cwd=os.path.dirname(com_file),
            shell=True,
        )
...
def subsequent_dft(
    oldchk,
    command="opt=(TS,noeigentest,calcfc) b3lyp/6-31+g(d,p) scrf=(smd,solvent=methanol) empiricaldispersion=gd3 Geom=AllCheckpoint",
    mem=2,
    cpus=4,
):
...
def compute_freq(
    oldchk,
    directory=".",
    command="freq external='~/soft/gau_xtb/xtb.sh' Geom=AllCheckpoint",
    mem=2,
    cpus=4,
):
...
def make_irc(oldchk, moldir):
    ...
    os.mkdir(reverse)
    reverse_com = write_irc_input_file(
        ts_chk, os.path.join(reverse, "revIRC"), "reverse"
    )
    orgpwd = os.getcwd()
    os.chdir(reverse)
    ...
    os.mkdir(forward)
    forward_com = write_irc_input_file(
        ts_chk, os.path.join(forward, "forIRC"), "forward"
    )
    orgpwd = os.getcwd()
    os.chdir(forward)
