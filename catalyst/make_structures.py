...
def ConstrainedEmbedMultipleConfs(
    ...
    if "." in Chem.MolToSmiles(mol):
        cids = AllChem.EmbedMultipleConfs(
            mol=mol, numConfs=numConfs, randomSeed=randomseed, numThreads=numThreads
        )  # jhj
    else:
        cids = AllChem.EmbedMultipleConfs(
            mol=mol,
...
if __name__ == "__main__":
    ...
    population = make_initial_population(
        5, "/home/julius/soft/GB-GA/ZINC_1000_amines.smi"
    )
    ...
    for cat in population:
        ...
        for i, ts2d in enumerate(ts2ds):  # for each constitutional isomer
            test = ConstrainedEmbedMultipleConfsMultipleFrags(
                ts2d, ts_dummy, numConfs=1, force_constant=1e3
            )
