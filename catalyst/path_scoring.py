...
def make_reactant(cat, ind_num, gen_num, n_confs=5, pruneRmsThresh=-1, randomseed=101, numThreads=1, warning_logger=None, directory='.'):
    ...
    for i, reactant2d in enumerate(reactants2d): # for each constitutional isomer
        ...
        while not bonds_ok and tries < max_tries:
            ...
            reactant3d = ConstrainedEmbedMultipleConfsMultipleFrags(mol=reactant2d_copy, core=reactant_dummy, numConfs=int(n_confs), randomseed=int(randomseed), numThreads=int(numThreads), force_constant=int(force_constant), pruneRmsThresh=pruneRmsThresh, atoms2join=[(11,29)])
    ...
    reactant_file = reactant3d_files[min_e_index] # lowest energy Reactant constitutional isomer
    # make gfn2 opt
    gfn2_opt_dir = os.path.join(os.path.dirname(os.path.dirname(reactant_file)), 'gfn2_opt_reactant')
    os.mkdir(gfn2_opt_dir)
    minE_reactant_regioisomer_file = os.path.join(gfn2_opt_dir, 'minE_reactant_isomer.xyz')
    shutil.move(reactant_file, minE_reactant_regioisomer_file)
    reactant, gfn2_reactant_energy = xtb_optimize(minE_reactant_regioisomer_file, method='gfn2', constrains='/home/julius/thesis/data/constr.inp', name=None, charge=cat_charge, scratchdir=directory, remove_tmp=False, return_file=False, numThreads=numThreads)
    return reactant
...
def calc_cat(reactant, ind_directory, cat_charge, numThreads):
    ...
    cat_opt_file, cat_energy = xtb_optimize(cat, name='catalyst', method='gfn2', charge=cat_charge, scratchdir=ind_directory, remove_tmp=False,return_file=True, numThreads=numThreads)
    return cat_energy
...
def make_product(reactant, cat_charge, cat, ind_num, gen_num, numThreads=1, directory='.'):
    ...
    rms = AllChem.AlignMol(catalyst, product_dummy, atomMap=(
        (cat_overlap[0], product_overlap[-1]), (cat_overlap[1], product_overlap[-2]), (cat_overlap[2], product_overlap[-3])))

    bs = [catalyst.GetBondBetweenAtoms(
        get_connection_id(catalyst), 0).GetIdx()]
    fragments_mol = Chem.FragmentOnBonds(
    ...
    product = connectMols(product_dummy, catalyst2, product_dummy.GetAtomWithIdx(
        product_dummyidx[0]), catalyst2.GetAtomWithIdx(cat2_dummyidx[0]))
    flags = Chem.SanitizeMol(product)

    product_opt, gfn2_reactant_energy = xtb_optimize(product, method='gfn2', constrains='/home/julius/thesis/data/constr.inp', name=f'G{gen_num:02d}_I{ind_num:02d}/gfn2_opt_product', charge=cat_charge, scratchdir=directory, remove_tmp=False, return_file=False, numThreads=numThreads)
    ...
    aligned_file = os.path.join(directory, f'G{gen_num:02d}_I{ind_num:02d}/gfn2_opt_product/aligned_product.xyz')
...
def xtb_path(ind_num, gen_num, charge, numThreads, inp_file='/home/julius/soft/GB-GA/catalyst/path_template.inp', directory='.'):
    ind_dir = os.path.join(directory, f'G{gen_num:02d}_I{ind_num:02d}')
    path_dir = os.path.join(ind_dir, 'path')
    os.mkdir(path_dir)
    shutil.copyfile(os.path.join(ind_dir, 'gfn2_opt_reactant/xtbopt.xyz'), os.path.join(path_dir, 'reactant.xyz'))
    shutil.copyfile(os.path.join(ind_dir, 'gfn2_opt_product/aligned_product.xyz'), os.path.join(path_dir, 'product.xyz'))
...
def run_sps(sp_dir, charge):
    for _file in os.listdir(sp_dir):
        if _file.endswith('.xyz'):
            p = subprocess.Popen(f'/home/julius/soft/xtb-6.3.3/bin/xtb {_file} --gfn2 --chrg {charge} --json --alpb methanol > {_file.split(".")[0]}.out', shell=True, cwd=sp_dir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            output, err = p.communicate()
            shutil.move(os.path.join(sp_dir, 'xtbout.json'), os.path.join(sp_dir, f'{_file.split(".")[0]}.json'))
...
def find_xtb_max_from_sp_interpolation(sp_directory, extract_max_structures):
    ...
    files = [os.path.join(sp_directory, f) for f in os.listdir(sp_directory) if \
             f.endswith("json")]
    ...
    if extract_max_structures:
        ...
        shutil.copy(os.path.join(sp_directory, 'sp_' + str(max_point-1)+'.xyz'), os.path.join(inter_dir, 'max_structure-1.xyz'))
        shutil.copy(os.path.join(sp_directory, 'sp_' + str(max_point)+'.xyz'), os.path.join(inter_dir, 'max_structure.xyz'))
        shutil.copy(os.path.join(sp_directory, 'sp_' + str(max_point+1)+'.xyz'), os.path.join(inter_dir, 'max_structure+1.xyz'))
        return inter_dir
    else:
        max_point = path_points[max_index]
...
def get_coordinates(interpolation_dir):
    ...
    for i in range(n_structures):
        atom_numbers = []
        with open(os.path.join(interpolation_dir,structure_files_list[i]), 'r') as struc_file:
...
def make_sp_interpolation(interpolation_dir, atom_numbers_list, coordinates_list, n_atoms, n_points):
    """
    ...
    """
    max_structure_files = ['max_structure-1.xyz', 'max_structure.xyz', 'max_structure+1.xyz']
    ...
    with open(os.path.join(interpolation_dir, 'path_file.txt'), 'w') as path_file:
        for i in range(n_structures-1):
            ...
            for j in range(n_points+1):
                ...
                file_path = os.path.join(interpolation_dir, 'path_point_' + str(i*n_points+j)+'.xyz')
