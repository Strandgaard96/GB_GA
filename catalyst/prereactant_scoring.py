...
def prereactant(
    ...
    for i, prereactant2d in enumerate(prereactant2ds):  # for each constitutional isomer
        force_constant = 1000
        prereactant3d = ConstrainedEmbedMultipleConfs(
            mol=prereactant2d,
            core=prereactant_dummy,
        ...
        # xTB optimite TS
        prereactant3d_file, prereactant3d_energy = xtb_optimize(
            prereactant3d,
            method="gfnff",
    ...
    prereactant_file = prereactant3d_files[
        min_e_index
    ]  # lowest energy TS constitutional isomer
    # make gfn2 opt
    gfn2_opt_dir = os.path.join(
        os.path.dirname(os.path.dirname(prereactant_file)), "gfn2_opt_prereactant"
    )
    os.mkdir(gfn2_opt_dir)
    minE_pre_regioisomer_file = os.path.join(
        gfn2_opt_dir, "minE_prereactant_isomer.xyz"
    )
    shutil.move(prereactant_file, minE_pre_regioisomer_file)
    minE_pre_regioisomer_opt, gfn2_pre_energy = xtb_optimize(
        minE_pre_regioisomer_file,
        method="gfn2",
        constrains=None,
        name=None,
        charge=cat_charge,
    ...
    os.mkdir(cat_path)
    cat_file = isolate_cat_from_xyz(
        minE_pre_regioisomer_opt, os.path.join(cat_path, "cat.xyz")
    )
    ...
    if not test_cat.HasSubstructMatch(cat):
        raise Exception(
            f"Change in BO or connectivity of Catalyst: {Chem.MolToSmiles(Chem.RemoveHs(Chem.MolToSmiles(test_cat)))} != {Chem.MolToSmiles(cat)}"
        )
    if not test_mol.HasSubstructMatch(prereactant_dummy):
        raise Exception(
            f"Change in BO or connectivity of Core: {Chem.MolToSmiles(Chem.RemoveHs(Chem.MolToSmiles(test_mol)))} != {Chem.MolToSmiles(cat)}"
        )
