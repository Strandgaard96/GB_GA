...
def ts_scoring(
    individual, args_list
):  # to be used in calculate_scores_parallel(population,final_scoring,[gen_num, n_confs, randomseed],n_cpus)
...
def activation_barrier(
    ...
    for i, ts2d in enumerate(ts2ds):  # for each constitutional isomer
        ...
        # xTB optimite TS
        ts3d_file, ts3d_energy = xtb_optimize(
            ts3d,
            method="gfn2",
    ...
    gfn2_opt_dir = os.path.join(
        os.path.dirname(os.path.dirname(ts_file)), "gfn2_opt_TS"
    )
    ...
    minE_TS_regioisomer_opt, gfn2_ts_energy = xtb_optimize(
        minE_TS_regioisomer_file,
        method="gfn2",
        constrains="/home/julius/thesis/data/constr.inp",
        name=None,
        charge=cat_charge,
    ...
    cat_path = os.path.join(
        directory,
        os.path.join(
            os.path.dirname(os.path.dirname(minE_TS_regioisomer_opt)), "catalyst"
        ),
    )
    os.mkdir(cat_path)
    cat_file = isolate_cat_from_xyz(
        minE_TS_regioisomer_opt, os.path.join(cat_path, "cat.xyz")
    )
