...
def ts_scoring(mol, args_list): # to be used in calculate_scores_parallel(population,final_scoring,[gen_num, n_confs, randomseed],n_cpus)
    ind_num, gen_num, n_confs, randomseed, logger, directory, n_cpus = args_list
    energy = activation_barrier(cat=mol, gen_num=gen_num, ind_num=ind_num, n_confs=n_confs, randomseed=randomseed, numThreads=n_cpus, logger=logger, directory=directory) 
    return energy

def activation_barrier(cat, ind_num, gen_num, n_confs=5, randomseed=101, numThreads=1, logger=None, directory='.'):
    if logger:
    ...
    for i, ts2d in enumerate(ts2ds): # for each constitutional isomer
        ...
        # xTB optimite TS
        ts3d_file, ts3d_energy = xtb_optimize(ts3d, name=f'G{gen_num:02d}_I{ind_num:02d}/const_iso{i:03d}', constrains='/home/julius/thesis/data/constr.inp', scratchdir=directory, remove_tmp=False, return_file=True, numThreads=numThreads)
    ...
    cat_opt_file, cat_energy = xtb_optimize(cat_file, name=None, scratchdir=directory, remove_tmp=False,return_file=True, numThreads=numThreads)
    if logger:
        elapsed_time = t1.stop()
        logger.info(f'{Chem.MolToSmiles(cat)} : {elapsed_time:0.4f} seconds')
