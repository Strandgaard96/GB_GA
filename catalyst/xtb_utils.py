...
def xtb_optimize(
    ...
    if isinstance(mol, Chem.rdchem.Mol):
        ...
        dest = os.path.join(os.path.abspath(scratchdir), name)
        os.makedirs(dest)
    ...
    for i, xyz_file in enumerate(xyz_files):
        ...
        p = subprocess.Popen(
            f"/home/julius/soft/xtb-6.3.3/bin/xtb --{method} {xyz_file} {opt_input} {constrains_input} {solvent_input} {charge_input} --json > out.out",
            shell=True,
...
def run_xtb_path(
    reactant_file,
    product_file,
    inp_file="/home/julius/thesis/data/path_template_allatoms.inp",
    charge=0,
    numThreads=1,
):
    os.environ["OMP_NUM_THREADS"] = f"{numThreads},1"
    ...
    p = subprocess.Popen(
        f"/home/julius/soft/xtb-6.3.3/bin/xtb {reactant_file} --path {product_file} --input {inp_file} --gfn2 --chrg {charge} --alpb methanol --verbose > xtb_path.out",
        shell=True,
        cwd=path_dir,
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
